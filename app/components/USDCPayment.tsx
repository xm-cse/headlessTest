...
import { crossmintCollectionId } from "@/lib/config";
...
export function USDCPayment({ onComplete }: USDCPaymentProps) {
  const [orderStatus, setOrderStatus] = useState<
    "creating" | "awaiting-payment" | "completed" | "processed" | "error"
  >("creating");
    ...
    try {
      const response = await fetch(`/api/orders/${orderDetails.orderId}`, {
        ...
        headers: {
          ...
          Authorization: orderDetails.clientSecret,
        },
        body: JSON.stringify({
          payment: {
            status: "processing",
  ...
  useEffect(() => {
    const createOrder = async () => {
      ...
      try {
        ...
        await checkOrderStatus(data.order.orderId);
      } catch (err) {
          ...
          <div className="bg-green-500/10 rounded-lg p-6 border border-green-500/20">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="font-mono text-sm break-all bg-zinc-900/50 p-2 rounded">
                  {hash}
                </p>
              </div>
              <button
          ...
          <div className="bg-green-500/10 rounded-lg p-6 border border-green-500/20">
            <div className="text-center">
              ...
              <p className="text-sm text-zinc-400">
                Check your inbox to find the minted NFT!
              </p>
            </div>
          </div>
        </div>
